...
def extract_steam_ids_from_file(file_path: str) -> List[str]:
    """
    파일에서 Steam ID를 추출
    
    Args:
        file_path: Steam ID가 포함된 파일 경로
    ...
    try:
        ...
        # Steam 프로필 URL에서 Steam ID 추출
        # 패턴: https://steamcommunity.com/profiles/76561198XXXXXXXXX/
        pattern = r'https://steamcommunity\.com/profiles/(\d+)/'
        matches = re.findall(pattern, content)
        
        # 중복 제거 및 정렬
        steam_ids = list(set(matches))
        steam_ids.sort()
        
        print(f"파일에서 {len(matches)}개의 URL을 찾았습니다.")
        print(f"중복 제거 후 {len(steam_ids)}개의 고유한 Steam ID를 추출했습니다.")
...
def main():
    ...
    # Steam ID 파일에서 추출
    steam_ids_file = "block_code.txt"
    steam_ids_to_block = extract_steam_ids_from_file(steam_ids_file)
    
    if not steam_ids_to_block:
        print("차단할 Steam ID를 찾을 수 없습니다.")
        print(f"{steam_ids_file} 파일을 확인해주세요.")
        return
    
    print("=== Steam 사용자 차단 도구 (파일에서 읽기) ===")
    ...
    # 처음 5개 Steam ID 미리보기
    print("처음 5개 Steam ID 미리보기:")
    for i, steam_id in enumerate(steam_ids_to_block[:5], 1):
        print(f"  {i}. {steam_id}")
    if len(steam_ids_to_block) > 5:
        print(f"  ... 외 {len(steam_ids_to_block) - 5}개")
    print()
